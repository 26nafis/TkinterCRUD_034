...
def fetch_all(_nama, bio, fis, ing, prediksi):
    conn = sqlite3.connect(DB_file)
    cur = conn.cursor()
    cur.execute('''
        INSERT INTO nilai_siswa (nama_siswa,
...
class NilaiApp:
    def __init__ (self, root):
        ...
        frm_right.grid(row=0, column=1, sticky='nswe', padx=12, pady=12)
        
        root.grid.columnconfigure(1, weight=1)
        root.grid.rowconfigure(0, weight=1)
        ttk.Label(frm_left, text='Nama_Siswa:', style = 'Header.TLabel').grid(row=0, column=0, sticky='w')
        self.entry_nama = ttk.Entry(frm_left, width=34)
        self.entry_nama.grid(row=1, column=0, pady=9, sticky='w')
